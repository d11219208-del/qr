...
# 初始化 Blueprint
# 注意：建議在 app.py 註冊時使用 app.register_blueprint(admin_bp, url_prefix='/admin')
...
def send_daily_report(manual_config=None, is_test=False):
    ...
    try:
        ...
        # 2. 時間區間處理 (台灣時間 00:00-23:59 轉 UTC)
...
總額：${x_total or 0}
---------------------------------
報告產出時間：{tw_now.strftime('%H:%M:%S')} (TW)
資料統計區間：今日 00:00 ~ 23:59
            """

        # 3. 呼叫 Resend API (忽略 SSL 憑證檢查以增加相容性)
        payload = {
            "from": sender_email,
            "to": [to_email],
            "subject": subject,
...
def async_send_report(app_instance, manual_config=None, is_test=False):
    """異步發送，避免網頁前端轉圈圈"""
    thread = threading.Thread(target=lambda: send_daily_report(manual_config, is_test))
    thread.start()
...
def admin_panel():
    ...
    if request.method == 'POST':
        ...
        if action == 'save_settings':
            keys = ['report_email', 'sender_email', 'resend_api_key']
            for k in keys:
        ...
        # 測試信
            ...
            send_daily_report(temp_config, True) # 測試信同步執行以便直接回傳結果
            return redirect(url_for('admin.admin_panel', msg="🧪 測試信已嘗試發送，請檢查信箱"))
